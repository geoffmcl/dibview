...
#ifdef	USELOCBUF
extern	QUANT_TBL_PTR	QTPtr[NUM_QUANT_TBLS];
extern	HUFF_TBL MLPTR HuffTPtrs[2*NUM_HUFF_TBLS];
extern	QUANT_VAL	QTbl1[DCTSIZE2];
#endif	/* USELOCBUF */
#endif	/* _INC_WINDOWS */
...
add_huff_table (compress_info_ptr cinfo,
				...
				const UINT8 MLPTR bits,
				const UINT8 MLPTR val)
{
	if (*htblptr == NULL)
	{
		*htblptr = (HUFF_TBL MLPTR)
			(*cinfo->emethods->alloc_small) (SIZEOF(HUFF_TBL));
	}

	MEMCOPY( (*htblptr)->bits,
		bits,
		SIZEOF((*htblptr)->bits) );
	MEMCOPY( (*htblptr)->huffval,
		val,
...
j_add_quant_table (compress_info_ptr cinfo, int which_tbl,
		   const QUANT_VAL MLPTR basic_table, int scale_factor,
 ...
 */
{
  QUANT_TBL_PTR FAR MLPTR qtblptr = & cinfo->quant_tbl_ptrs[which_tbl];
  int i;
  long temp;
...
/* NB: the external methods must already be set up. */
{
  ...
  cinfo->comp_info = NULL;
	...
	{
		//memset( (void MLPTR)QTPtr[i], 0, sizeof(QTbl1) );	/* Zero table */
...
#ifdef	_INC_WINDOWS
  cinfo->jpeg_color_space = LibConfig.ccf_jpeg_color;	/* Def=CS_YCbCr */
  cinfo->num_components = LibConfig.ccf_num_components;	/* Def = 3 */
#else
  cinfo->jpeg_color_space = CS_YCbCr;
  cinfo->num_components = 3;
#endif

  cinfo->comp_info = (jpeg_component_info MLPTR)
...
#ifdef	_INC_WINDOWS
  compptr->h_samp_factor = LibConfig.ccf_h_samp_factor;	/* default to 2x2 */
  compptr->v_samp_factor = LibConfig.ccf_v_samp_factor;	/* subsamples of chrominance */
	/* The above would be 1x1 if monochrome ... */
#else
  ...
  cinfo->optimize_coding = LibConfig.ccf_optimize_coding;	/* Def=FALSE */
  /* No input smoothing */
  cinfo->smoothing_factor = LibConfig.ccf_smoothing_factor;	/* Def=0 */
  /* No restart markers */
  cinfo->restart_interval = LibConfig.ccf_restart_intevals;	/* Def=0 */
  cinfo->restart_in_rows = LibConfig.ccf_restart_in_rows;	/* Def=0 */
#else
  /* By default, don't do extra passes to optimize entropy coding */
