...
/* On entry, cinfo->comps_in_scan and cinfo->cur_comp_info[] are set up */
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_JPGICA ); /* "I'm confused about the image width" */
		return;
#else
    ...
    if( cinfo->blocks_in_MCU + mcublks > MAX_BLOCKS_IN_MCU )
	 {
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_JPGSFT ); /* "Sampling factors too large for interleaved scan" */
		return;
#else
...
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; ci < cinfo->num_components; ci++) {
    ...
    (*cinfo->emethods->free_small) ((void MLPTR) fullsize_data[1][ci]);
  }

  /* Free the top-level space */
  (*cinfo->emethods->free_small) ((void MLPTR) fullsize_data[0]);
  (*cinfo->emethods->free_small) ((void MLPTR) fullsize_data[1]);
}
...
// #pragma	message( "Static allocation of MCU_data!!!" )
...
METHODDEF void
MCU_output_catcher (compress_info_ptr cinfo, JBLOCK MLPTR MCU_data)
...
{
#ifdef	_INC_WINDOWS
	ERRFLAG( BAD_NOTIMP );	/* "Not implemented yet" */
#else
...
#else
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_JPGMSS ); /* "Multiple-scan support was not compiled" */
#else
