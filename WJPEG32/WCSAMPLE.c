...
{
  jpeg_component_info MLPTR compptr = cinfo->cur_comp_info[which_component];
...
jseldownsample (compress_info_ptr cinfo)
{
  ...
  boolean smoothok = TRUE;
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_CCIR ); /* "CCIR601 downsampling not implemented yet" */
		return;
#else
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	cinfo->methods->downsample[ci] = h2v2_downsample;
	 ...
	 {
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_FRACTD ); /* "Fractional downsampling not implemented yet" */
		return;
#else
