...
LOCAL int
slow_DECODE (HUFF_TBL MLPTR htbl, int min_bits)
{
...
process_restart (decompress_info_ptr cinfo)
{
  ...
  do {
	...
	{
#ifdef	_INC_WINDOWS
		WARNFLAG( BAD_JPGDAT2 ); /* "WARNING: Corrupt JPEG data: Extraneous bytes before marker" */
#else
...
METHODDEF void
decode_mcu (decompress_info_ptr cinfo, JBLOCKROW MLPTR MCU_data)
{
  register int s, k, r;
  short blkn, ci;
  register JBLOCKROW block;
  register QUANT_TBL_PTR quanttbl;
  HUFF_TBL MLPTR dctbl;
  HUFF_TBL MLPTR actbl;
  jpeg_component_info MLPTR compptr;
