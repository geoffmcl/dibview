...
static big_sarray_ptr MLPTR fullsize_image;
...
{
  jpeg_component_info MLPTR compptr = cinfo->cur_comp_info[0];
...
/* Release a sample image created by alloc_sampimage */
{
  ...
  (*cinfo->emethods->free_small) ((void MLPTR) image);
}
...
LOCAL void
alloc_sampling_buffer (decompress_info_ptr cinfo, JSAMPIMAGE s_data[2])
...
LOCAL void
free_sampling_buffer (decompress_info_ptr cinfo, JSAMPIMAGE sp_data[2])
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    (*cinfo->emethods->free_small_sarray) (sp_data[0][ci]);
    /* Free the scrambled-order pointers */
    (*cinfo->emethods->free_small) ((void MLPTR) sp_data[1][ci]);
  }

  /* Free the top-level space */
  (*cinfo->emethods->free_small) ((void MLPTR) sp_data[0]);
  (*cinfo->emethods->free_small) ((void MLPTR) sp_data[1]);
}
...
/* Allocate and fill in the sample_range_limit table */
{
  ...
  table = (JSAMPLE MLPTR) (*cinfo->emethods->alloc_small)
...
JSAMPROW exdummy[MAX_SAMP_FACTOR]; /* for downsample expansion at top/bottom */

LOCAL void
expand (decompress_info_ptr cinfo,
	JSAMPIMAGE sa_data, JSAMPIMAGE fullsize_data,
	long fullsize_width,
...
/* Special case: above, below can be -1 to indicate top, bottom of image. */
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (above >= 0)
      above_ptr = sa_data[ci] + above * vs;
    else {
      /* Top of image: make a dummy above-context with copies of 1st row */
      /* We assume current=0 in this case */
      for (i = 0; i < vs; i++)
	exdummy[i] = sa_data[ci][0];
      above_ptr = (JSAMPARRAY) exdummy; /* possible near->far pointer conv */
    }

    if (below >= 0)
      below_ptr = sa_data[ci] + below * vs;
    else {
      /* Bot of image: make a dummy below-context with copies of last row */
      for (i = 0; i < vs; i++)
	exdummy[i] = sa_data[ci][(current+1)*vs-1];
      below_ptr = (JSAMPARRAY) exdummy; /* possible near->far pointer conv */
    }
...
/* The output row is placed in coeff_data.  bsm and whb hold */
...
get_smoothed_row (decompress_info_ptr cinfo, JBLOCKIMAGE coeff_data,
		  JBLOCKIMAGE bsm[3], int MLPTR whb, long cur_mcu_row)
{
  int prev, cur, next;
  
  /* Special case for top of image: need to pre-fetch a row & init whb */
  if (cur_mcu_row == 0) {
    (*cinfo->methods->disassemble_MCU) (cinfo, bsm[0]);
    if (cinfo->MCU_rows_in_scan > 1) {
      (*cinfo->methods->disassemble_MCU) (cinfo, bsm[1]);
      smooth_mcu_row(cinfo, (JBLOCKIMAGE) NULL, bsm[0], bsm[1],
		     coeff_data);
    } else {
      smooth_mcu_row(cinfo, (JBLOCKIMAGE) NULL, bsm[0], (JBLOCKIMAGE) NULL,
		     coeff_data);
    }
    *whb = 1;		/* points to next bsmooth[] element to use */
    return;
  }
  
  cur = *whb;		/* set up references */
  prev = (cur == 0 ? 2 : cur - 1);
  next = (cur == 2 ? 0 : cur + 1);
  *whb = next;		/* advance whichb for next time */
  
  /* Special case for bottom of image: don't read another row */
  if (cur_mcu_row >= cinfo->MCU_rows_in_scan - 1) {
    smooth_mcu_row(cinfo, bsm[prev], bsm[cur], (JBLOCKIMAGE) NULL,
		   coeff_data);
    return;
  }
  
  /* Normal case: read ahead a new row, smooth the one I got before */
  (*cinfo->methods->disassemble_MCU) (cinfo, bsm[next]);
  smooth_mcu_row(cinfo, bsm[prev], bsm[cur], bsm[next],
		 coeff_data);
}
...
#endif
  /* Work buffer for downsampled image data (see comments at head of file) */
//  JSAMPIMAGE sampled_data[2];	/* Allocated in DS, not STACK for library! */
  int whichss, ri;
  short ci, i;
  boolean single_scan;
  ...
  fullsize_image = (big_sarray_ptr MLPTR) (*cinfo->emethods->alloc_small)
