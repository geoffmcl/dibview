 ...
 * at the start of the file; t_input_init may need work if the
 * user interface has already read some data (e.g., to determine that
 * the file is indeed Targa format).
 *
 * These routines are invoked via the methods get_input_row
 * and t_input_init/term.
 *
...
/* Read next byte from Targa file */
{
  ...
  register int c;
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TAREOF );	/* "Unexpected end of Targa file!" */
#else
		ERREXIT(cinfo->emethods, "Unexpected end of Targa file!");
#endif
	}
return( c );
}
...
/* Read the colormap from a Targa file */
{
  ...
  /* Presently only handles 24-bit BGR format */
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARCF );	/* "Unsupported Targa colormap format" */
#else
...
METHODDEF boolean
t_input_init (compress_info_ptr cinfo)
{
...
#define GET_2B(offset)	((unsigned int) UCH(targaheader[offset]) + \
  ...
  if (! ReadOK(cinfo->input_file, targaheader, 18))
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TAREOF );	/* "Unexpected end of file" */
		return( bErrFlag );
#else
  ...
  if (cmaptype > 1 ||		/* cmaptype must be 0 or 1 */
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARGA );	/* "Invalid or unsupported Targa file" */
		return( bErrFlag );
#else
  ...
  if (maplen > 0) {
    if (maplen > 256 || GET_2B(3) != 0)
	 {
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARSIZ );	/* "Colormap too large" */
		return( bErrFlag );
#else
...
{
  cinfo->methods->input_init = t_input_init;
  /* cinfo->methods->get_input_row is set by t_input_init */
