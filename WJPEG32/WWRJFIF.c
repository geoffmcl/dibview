...
#undef	USEWRBUF		// Use a WRITE buffer of 1K
...
/* Write a single byte */
#ifdef	_INC_WINDOWS
#if	(defined( WJPEG4 ) || (defined( WIN32 ) && defined( USEWRBUF )))

extern int b_putc( int c, HFILE Hand );
LOCAL	void
emit_byte(compress_info_ptr cinfo, int c )
{
	b_putc( c, cinfo->output_file );
}
...
BYTE	buf[2];
	buf[0] = (BYTE) c;
	if( _lwrite( cinfo->output_file, &buf[0], 1 ) != 1 )
	{
		ERRFLAG( BAD_WWRITE );	/* Output file error ... */
	}
}
#endif	// !WJPEG4
...
// WHERE IS THIS USED??? IT SHOULD BE RETIRED ALSO!!!
#define CHECK_OUTPUT(cinfo)	\
	{ if( cinfo->output_file && ( cinfo->output_file != HFILE_ERROR ))	\
...
#define CHECK_OUTPUT(cinfo)	\
	{ if( cinfo->output_file )	\
		{	b_fflush( cinfo->output_file); _lclose( cinfo->output_file ); }	\
		cinfo->output_file = 0; }
#endif	// WJPEG4 y/n

#define FLUSH_OUTPUT(cinfo)	\
{\
	if( cinfo->output_file &&\
...
/* Emit a DHT marker */
{
  ...
  if (is_ac) {
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_HUFFTAB ); /* "Huffman table 0x%02x was not defined" */
		return;
#else
...
/* Emit a SOF marker */
{
  ...
  emit_2bytes(cinfo, 3 * cinfo->num_components + 2 + 5 + 1); /* length */
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_JFIFMAX );	/* "Maximum image dimension for JFIF is 65535 pixels" */
		return;
#else
...
// Emit a COM marker.
LOCAL void
emit_com (compress_info_ptr cinfo, char MLPTR dataptr, size_t datalen)
{
	if( datalen &&
		( (unsigned) datalen <= (unsigned) 65533 ) )
	{
		// safety checked
...
// FIX16 - 11
METHODDEF void
write_jpeg_data (compress_info_ptr cinfo, char MLPTR dataptr, int datacount)
{
	int		i;
	PINT8	pd;
	...
	if( datacount &&
		( pd = (PINT8)dataptr ) )
	{
		// No flush since all to same BUFFER
		//FLUSH_OUTPUT( cinfo );
		for( i = 0; i < datacount; i++ )
		{
			emit_byte( cinfo, pd[i] );
		}
	}
//	HFILE	hf;
//	if( JFWRITE(cinfo->output_file, dataptr, datacount) != 
...
//		( hf = cinfo->output_file ) &&
//		( hf != HFILE_ERROR ) )
//	{
//		// Must FLUSH before this I/O
//		FLUSH_OUTPUT( cinfo );
//		if( gout_write( hf, dataptr, datacount) != 
//			(size_t) (datacount))
//		{
//#ifdef	_INC_WINDOWS
//			ERRFLAG( BAD_WRITOUT );	/* "Output file write error --- out of disk space?" */
//#else
//		ERREXIT(cinfo->emethods, "Output file write error --- out of disk space?");
//#endif
//		}
