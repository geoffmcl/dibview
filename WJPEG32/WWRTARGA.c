...
LOCAL void
t_write_header (decompress_info_ptr cinfo, int num_colors)
/* Create and write a Targa header */
{
  ...
  if (JFWRITE(cinfo->output_file, targaheader, 18) != (size_t) 18)
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARHDR ); /* "Could not write Targa header" */
#else
...
METHODDEF boolean
t_output_init (decompress_info_ptr cinfo)
{
  if (cinfo->out_color_space == CS_GRAYSCALE) {
    /* Targa doesn't have a mapped grayscale format, so we will */
    /* demap quantized gray output.  Never emit a colormap. */
    t_write_header(cinfo, 0);
#ifndef USE_PUTC_OUTPUT
...
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARRGB ); /* "Targa output must be grayscale or RGB" */
		return( bErrFlag );
#else
...
#ifdef	_INC_WINDOWS
    (void) JFWRITE(outfile, row_buffer, (UINT) (3*width));
#else
...
#ifdef	_INC_WINDOWS
    (void) JFWRITE(outfile, row_buffer, (UINT) width);
#else
...
put_color_map (decompress_info_ptr cinfo, int num_colors, JSAMPARRAY colormap)
{
  ...
  if (cinfo->out_color_space == CS_RGB) {
    ...
    if( num_colors > 256 )
	 {
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_TARCOLS );	/* "Too many colors for Targa output" */
		return;
#else
    ...
    t_write_header(cinfo, num_colors);
...
{
  cinfo->methods->output_init = t_output_init;
