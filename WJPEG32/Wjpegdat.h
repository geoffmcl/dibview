...
#endif // #ifndef DV322
#else
typedef BYTE huge * PINT8;
#endif
#endif // !_MSC_VER

#ifdef	_INC_WINDOWS
typedef HFILE JFILEREF;
typedef HFILE IFILEREF;
#else	/* !_INC_WINDOWS */
 ...
 * similarly-named fields in Compress_info_struct and Decompress_info_struct1.
...
#ifdef	_INC_WINDOWS
#define	JPTR	FAR *
typedef struct External_methods_struct JPTR external_methods_ptr;
typedef struct Compress_methods_struct JPTR compress_methods_ptr;
typedef struct Decompress_methods_struct JPTR decompress_methods_ptr;
typedef struct Compress_info_struct JPTR compress_info_ptr;
typedef struct Decompress_info_struct1 JPTR decompress_info_ptr;
#else
...
typedef struct Decompress_info_struct1 * decompress_info_ptr;
#endif
...
#ifndef	_INC_WINDOWS	/* These moved to WJPEGLIB.H for Windows */
...
typedef QUANT_VAL MLPTR QUANT_TBL_PTR;	/* pointer to same */
 ...
 */
...
#ifdef	_INC_WINDOWS
	short in_color_space; /* colorspace of input file */
	short jpeg_color_space; /* colorspace of JPEG file */
#else
	...
	char MLPTR comment_text;	/* Text for COM block, or NULL for no COM */
	...
	HUFF_TBL MLPTR dc_huff_tbl_ptrs[NUM_HUFF_TBLS];
	HUFF_TBL MLPTR ac_huff_tbl_ptrs[NUM_HUFF_TBLS];
...
#ifdef	WJPEG4
	boolean	progressive;	// Progressive mode
#endif	// WJPEG4
...
#ifdef	_INC_WINDOWS
	HGLOBAL		hgOut;	/* Output GLOBAL Object handle */
	PINT8		lpOut;	/* Output pointer ... */
	DWORD		ddOut;
#ifdef	WJPEG4
	JSAMPARRAY	pcolormap;	// Pointer to COLOR map array
	GIFHDREXT	GHE;
#endif	// WJPEG4
#endif
};
...
// Working data for decompression - If possible should now use
// the NEWER stucture on Version 61 (or 6a) of JPEG software
// ===========================================================
struct Decompress_info_struct1 {
/*
 ...
 */
	char MLPTR input_buffer;	/* start of buffer (private to input code) */
	char MLPTR next_input_byte;	/* => next byte to read from buffer */
...
typedef struct big_sarray_control MLPTR big_sarray_ptr;
typedef struct big_barray_control MLPTR big_barray_ptr;
...
struct External_methods_struct {
	 ...
	 */
	METHOD(void, error_exit, (const char MLPTR msgtext));
	METHOD(void, trace_message, (const char MLPTR msgtext));
	...
	METHOD(void MLPTR, alloc_small, (size_t sizeofobject));
	METHOD(void, free_small, (void MLPTR ptr));
	METHOD(void MLPTR, alloc_medium, (size_t sizeofobject));
	METHOD(void, free_medium, (void MLPTR ptr));
...
#ifndef	_INC_WINDOWS	// Eventuall do this ...
/* === Excluded from WINDOWS === */
...
struct Compress_methods_struct {
...
#ifdef	PROGRESS_REPORT	/* Only if defined ... */
	...
	METHOD(boolean, input_init, (compress_info_ptr cinfo));
...
struct Decompress_methods_struct {
	...
	METHOD(boolean, output_init, (decompress_info_ptr cinfo));
...
#ifdef	_INC_WINDOWS
extern void j_add_quant_table( compress_info_ptr, int, 
				  const QUANT_VAL MLPTR,
				  int, boolean );
#else
...
EXTERN void jselrbmp PP((compress_info_ptr cinfo)); /* jrdbmp.c */
...
#ifdef	ADD_BMP
EXTERN void jselwbmp PP((decompress_info_ptr cinfo)); /* jwrbmp.c */
#endif	/* ADD_BMP */
