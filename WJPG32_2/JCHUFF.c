...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	entropy->dc_count_ptrs[dctbl] = (long MLPTR)
	  (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				      257 * SIZEOF(long));
      MEMZERO(entropy->dc_count_ptrs[dctbl], 257 * SIZEOF(long));
      if (entropy->ac_count_ptrs[actbl] == NULL)
	entropy->ac_count_ptrs[actbl] = (long MLPTR)
	  (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				      257 * SIZEOF(long));
...
GLOBAL(void)
jpeg_make_c_derived_tbl (j_compress_ptr cinfo, JHUFF_TBL MLPTR htbl,
			 c_derived_tbl MLPTR MLPTR pdtbl)
{
  c_derived_tbl MLPTR dtbl;
  int p, i, l, lastp, si;
  char huffsize[257];
...
LOCAL(boolean)
dump_buffer (working_state MLPTR state)
/* Empty the output buffer; return TRUE if successful, FALSE if must suspend */
{
  struct jpeg_destination_mgr MLPTR dest = state->cinfo->dest;
...
INLINE
LOCAL(boolean)
emit_bits (working_state MLPTR state, unsigned int code, int size)
...
LOCAL(boolean)
flush_bits (working_state MLPTR state)
{
...
LOCAL(boolean)
encode_one_block (working_state MLPTR state, JCOEFPTR block, int last_dc_val,
		  c_derived_tbl MLPTR dctbl, c_derived_tbl MLPTR actbl)
{
...
LOCAL(boolean)
emit_restart (working_state MLPTR state, int restart_num)
{
  int ci;
...
METHODDEF(boolean)
encode_mcu_huff (j_compress_ptr cinfo, JBLOCKROW MLPTR MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  working_state state;
  int blkn, ci;
...
METHODDEF(boolean)
encode_mcu_gather (j_compress_ptr cinfo, JBLOCKROW MLPTR MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int blkn, ci;
  jpeg_component_info MLPTR compptr;
...
GLOBAL(void)
jpeg_gen_optimal_table (j_compress_ptr cinfo, JHUFF_TBL MLPTR htbl, long freq[])
{
...
jinit_huff_encoder (j_compress_ptr cinfo)
{
  ...
  cinfo->entropy = (struct jpeg_entropy_encoder MLPTR ) entropy;
