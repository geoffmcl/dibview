 ...
 *
 * These routines are invoked via the methods gd_put_pixel_rows, gd_put_color_map,
 * and gd_output_init/term.
 */
...
#include	"dv.h"
#include	"DvB2g.h"	// Was bmp2gif.h

#ifdef GIF_SUPPORTED

#ifdef	BMP2GIF
extern void	DV_putc( int, HFILE );
extern void	DV_fflush( HFILE );
extern int b_write( HFILE, LPSTR, int );
#define	JFWRITE( a, b, c )	b_write( a, b, c )
void	DV_zero_far( PINT8 hf, DWORD len )
{
	dv_fmemset( hf, 0, len );
}
DWORD	bErrFlag = 0;
#if (defined( _INC_WINDOWS ) && defined( BMP2GIF ))
HGLOBAL	hhcode = 0;
HGLOBAL	hhpref = 0;
HGLOBAL	hhsuff = 0;
#endif
...
static code_int MLPTR hash_code;	/* => hash table of symbol codes */
static code_int MLPTR hash_prefix; /* => hash table of prefix symbols */
static UINT8 MLPTR hash_suffix;	/* => hash table of suffix bytes */
...
#else
#ifdef	_INC_WINDOWS
#define CHAR_OUT(c)  DV_putc( (int) c, dcinfo->output_file )
#else
/*
 * Routines to package compressed data bytes into GIF data blocks.
 * A data block consists of a count byte (1..255) and that many data bytes.
...
/* Fill the hash table with empty entries */
{
  ...
  DV_zero_far((void MLPTR) hash_code, HSIZE * SIZEOF(code_int));
}
...
{
  DV_putc(w & 0xFF, dcinfo->output_file);
  DV_putc((w >> 8) & 0xFF, dcinfo->output_file);
}
...
{
  DV_putc(val, dcinfo->output_file);
  DV_putc(val, dcinfo->output_file);
  DV_putc(val, dcinfo->output_file);
}
...
LOCAL void
emit_GIF_header (int num_colors, JSAMPARRAY colormap)
{
  ...
  int i;
	...
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_GIF256 ); /* "GIF can only handle 256 colors" */
		return;
#else
   ...
   */
  DV_putc('G', dcinfo->output_file);
  DV_putc('I', dcinfo->output_file);
  DV_putc('F', dcinfo->output_file);
  DV_putc('8', dcinfo->output_file);
  DV_putc('7', dcinfo->output_file);
  DV_putc('a', dcinfo->output_file);
  ...
  DV_putc(FlagByte, dcinfo->output_file);
  DV_putc(0, dcinfo->output_file);	/* Background color index */
  DV_putc(0, dcinfo->output_file);	/* Reserved in GIF87 (aspect ratio in GIF89) */
  ...
  /* we reduce it to 8 bits by shifting */
	...
	else	/* We HAVE a colormap (Array 3 components by num_colors) ... */
	{
		if( dcinfo->out_color_space == CS_RGB )
		{	/* Normal case: RGB color map */
			for( ; i < num_colors; i++ )
			{
				DV_putc(GETJSAMPLE(colormap[0][i]) >> cshift, dcinfo->output_file);
				DV_putc(GETJSAMPLE(colormap[1][i]) >> cshift, dcinfo->output_file);
				DV_putc(GETJSAMPLE(colormap[2][i]) >> cshift, dcinfo->output_file);
			}
		}
		else	/* We have a GRAY SCALE */
		{
			for( ; i < num_colors; i++ )
	...
	{
		put_3bytes(0);	/* fill out the map to a power of 2 */
	}
  /* Write image separator and Image Descriptor */
  DV_putc(',', dcinfo->output_file); /* separator */
  put_word((UINT16) 0);		/* left/top offset */
  put_word((UINT16) 0);
  put_word((UINT16) dcinfo->image_width); /* image size */
  put_word((UINT16) dcinfo->image_height);
  /* flag byte: not interlaced, no local color map */
  DV_putc(0x00, dcinfo->output_file);
  /* Write Initial Code Size byte */
  DV_putc(InitCodeSize, dcinfo->output_file);
...
METHODDEF boolean
gd_output_init (decompress_info_ptr cinfo)
{
  dcinfo = cinfo;		/* save for use by local routines */
	if( cinfo->final_out_comps != 1 ) /* safety check */
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_GIFOUT );	/* "GIF output got confused" */
		goto goutret;
#else
...
#if (defined( _INC_WINDOWS ) && defined( BMP2GIF ))
	if( ( hhcode = DVGlobalAlloc( GHND, (HSIZE * SIZEOF(code_int)) ) ) &&
		( hhpref = DVGlobalAlloc( GHND, (HSIZE * SIZEOF(code_int)) ) ) &&
		( hhsuff = DVGlobalAlloc( GHND, (HSIZE * SIZEOF(UINT8)) ) ) &&
		( hash_code = (code_int MLPTR) DVGlobalLock( hhcode ) ) &&
  		( hash_prefix = (code_int MLPTR) DVGlobalLock( hhpref ) ) &&
		( hash_suffix = (UINT8 MLPTR) DVGlobalLock( hhsuff ) ) )
	{

	}
#else
  hash_code = (code_int MLPTR) (*cinfo->emethods->alloc_medium)
				(HSIZE * SIZEOF(code_int));
  hash_prefix = (code_int MLPTR) (*cinfo->emethods->alloc_medium)
				(HSIZE * SIZEOF(code_int));
  hash_suffix = (UINT8 MLPTR) (*cinfo->emethods->alloc_medium)
				(HSIZE * SIZEOF(UINT8));
#endif
  /*
   * If we aren't quantizing, gd_put_color_map won't be called,
   ...
   */
	...
	{
		emit_GIF_header(256, (JSAMPARRAY) NULL);
	}
#ifdef	_INC_WINDOWS
goutret:
#endif
   return((boolean) bErrFlag );
}
...
METHODDEF void
gd_put_color_map (decompress_info_ptr cinfo, int num_colors, JSAMPARRAY colormap)
{
  emit_GIF_header(num_colors, colormap);
}
...
METHODDEF void
gd_put_pixel_rows (decompress_info_ptr cinfo, int num_rows,
		JSAMPIMAGE pixel_data)
{
  ...
  register int row;
	...
	for( row = 0; row < num_rows; row++ )
	{
		...
		if( ptr == 0 )
		{
			ERRFLAG( BAD_GIFPTR );	/* ERROR: NULL Data pointer passed! */
		}
#endif
...
METHODDEF void
gd_output_term (decompress_info_ptr cinfo)
{
  /* Flush LZW mechanism */
  compress_term();
  /* Write a zero-length data block to end the series */
  DV_putc(0, cinfo->output_file);
  /* Write the GIF terminator mark */
  DV_putc(';', cinfo->output_file);
  /* Make sure we wrote the output file OK */
#ifdef	_INC_WINDOWS
	DV_fflush( cinfo->output_file );
	if( cinfo->output_file )
		DVlclose(IntToPtr(cinfo->output_file));
	cinfo->output_file = 0;
#else
...
#ifdef	BMP2GIF
	if( hhcode && hash_code )
		DVGlobalUnlock( hhcode );
	if( hhpref && hash_prefix )
		DVGlobalUnlock( hhpref );
	if( hhsuff && hash_suffix )
		DVGlobalUnlock( hhsuff );
	if( hhcode )
		DVGlobalFree( hhcode );
	if( hhpref )
		DVGlobalFree( hhpref );
...
GLOBAL void
DVselwgif (decompress_info_ptr cinfo)
{
  cinfo->methods->output_init = gd_output_init;
  cinfo->methods->put_color_map = gd_put_color_map;
  cinfo->methods->put_pixel_rows = gd_put_pixel_rows;
  cinfo->methods->output_term = gd_output_term;

  if( cinfo->out_color_space != CS_GRAYSCALE &&
      cinfo->out_color_space != CS_RGB )
	{
#ifdef	_INC_WINDOWS
		ERRFLAG( BAD_GIFNRGB );	/* "GIF output must be grayscale or RGB" */
		return;
#else
